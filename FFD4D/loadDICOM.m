function [s_Images] = loadDICOM(sFolder)
% LOADDICOM M-file
...
for i=1:iFileNum
    ...
    try
        ...
        % Grab image
        curImage.ImgData = dicomread([sFolder '\' sCurFName]);
        s_Images(i) = curImage;
    catch
...
if (~exist('s_Images'))
    s_Images = [];      % return empty variable
    return
end

% Sort the images by slice number (insertion)
for i=2:size(s_Images,2)
    index = s_Images(i).SliceLocation;
    temp = s_Images(i);
    j = i;
    while ( (j>1) && (s_Images(j-1).SliceLocation > index) )
        s_Images(j) = s_Images(j-1);
        j = j-1;
    end
