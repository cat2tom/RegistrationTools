function contour = getRTOGcont(fullpath,segname,lim)
...
fid = fopen(fullpath);
 ...
 % Clear the white spaces in front of some field names
    ...
    text = sliceInfo{8};
    contour = zeros(100,100,str2num(text(find(text=='=')+1:length(text))));
    ...
    text = fgetl(fid);
    levels = str2num(text(find(text==':')+2:length(text)));
       ...
       while 1 
            text = fgetl(fid);
                        if ~isempty(findstr('Scan Number',text)) || isempty(text)
                            n = n_lim - str2double(text(find(text==':')+2:length(text)))+1;
                             text = fgetl(fid);
        ...
        if str2double(text(find(text==':')+2:length(text))) ~= 0
            ...
            if (text~=-1)         % Haven't reached EOF
                ...
                C = [];
                            ...
                            text = fgetl(fid);
                            points = str2double(text(find(text==':')+2:length(text)));
                            for m = 1:segments 
                            i = 0;
                            while 1 
                                i = i+1;
                                layer{i} = fgetl(fid); 
                                if ~isempty(findstr('"',layer{i}))
...
%                             %n',points,'Delimiter',',');
                           ...
                           if ~isempty(C)
                            ...
                            X = round((C(:,1)+xoff)./res(2)+(dim1/2))-lim(3)+1;
                            Y = round(-(C(:,2)-yoff)./res(1)+(dim2/2))-lim(1)+1;                         
                            if length(X) > 2 && ~isempty(C)
                            sz = [lim(2)-lim(1)+1,lim(4)-lim(3)+1];    
                            X2 = round((interp1(1:length(X),X,1:0.05:length(X))).*(99/(sz(2)-1)));                   
                            Y2 = round((interp1(1:length(Y),Y,1:0.05:length(Y))).*(99/(sz(1)-1)));
                            X3 = X2;
                            Y3 = Y2;
                            X3(X2 <= 0 | X2 > 100 | Y2 <= 0 | Y2 > 100) = [];
                            Y3(X2 <= 0 | X2 > 100 | Y2 <= 0 | Y2 > 100) = [];
                            Y2 = Y3;
                            X2 = X3;
                            clear X3 Y3
