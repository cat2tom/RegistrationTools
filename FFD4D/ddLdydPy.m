...
%control point
function values = ddLdydPy(base,sz2,sz,m,n,i1,j1)
sz3 = size(base);
[y,x] = find(base >= 0);
...
%             u = X(indices).*sz2(2)./sz(2) - floor(X(indices).*sz2(2)./sz(2));
%             v = Y(indices).*sz2(1)./sz(1) - floor(Y(indices).*sz2(1)./sz(1));
...
%             v = rem(Y(indices),(sz(1)/((sz2(1)-3))+1));
            ...
            ysum = bspline(u,n-j(indices)).*dbspline(v,m-i(indices)).*dvdy;
