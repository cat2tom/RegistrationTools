...
%form deformation by the change in z.
function [xsum,ysum,zsum] = dLdz3D(sz,dgridx,dgridy,dgridz)
...
dwdz = (sz2(3)-3)/sz(3);
        ...
        for q = 0:3
            ...
            for l = 0:3   
                ...
                c = dbspline2(w,r);
                xproduct = a.*b.*c.*dgridx(ind).*dwdz;
                yproduct = a.*b.*c.*dgridy(ind).*dwdz;
                zproduct = a.*b.*c.*dgridz(ind).*dwdz;
                xsum = xsum + xproduct;
                ysum = ysum + yproduct;
                zsum = zsum + zproduct;
                end
